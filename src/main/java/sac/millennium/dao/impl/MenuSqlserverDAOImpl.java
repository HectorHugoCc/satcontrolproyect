...
public class MenuSqlserverDAOImpl implements IMenuDAO, Serializable {
	...
	public int create(Menu obj) {
		...
		System.out.println("Nuevo Menu registrado >>> " + obj.getFormularioAsociado() + obj.getOrdenAparicion());
		return estado;
	}
	...
	public List<Menu> listaContenedoresHermanos() {
		...
		Menu obj = null;
		try {
			String sql = "select * from menu where contenedor_menu = id_menu and formulario_asociado_menu = '#' order by orden_aparicion_menu asc;";
			pstm = cx.prepareStatement(sql);
			rs = pstm.executeQuery();
			while (rs.next()) {
				obj = new Menu();
	...
	public List<Menu> listaContenedoresHermanos(Menu menu) {
		...
		Menu obj = null;
		try {
			String sql = "select * from menu where contenedor_menu = ? and formulario_asociado_menu = '#' order by orden_aparicion_menu asc;";
			pstm = cx.prepareStatement(sql);
